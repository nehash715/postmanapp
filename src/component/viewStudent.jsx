...
state={
 ...
 async fetchData(){
    ...
    let response= await http.get(`/students?page=${page}&${searchStr}`)
...
this.fetchData()
    ...
    handlePage=(incr)=>{
        ...
        this.callURL("/viewStudents",queryParams)

    }
    handleOption=(options)=>{
    this.callURL("/viewStudents",options)
    }
    callURL=(url,options)=>{
...
<div className="col-3">
    <StudentPanel  options={queryParams}   onOptionChange={this.handleOption}/>
</div>
<div className="col-9">
...
{data.items?.map((p)=>{
    ...
    <div className="col-2">{p.dob}</div>
    <div className="col-3">{p.about}</div>
